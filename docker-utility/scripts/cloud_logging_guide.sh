...
show_log_aggregation() {
    ...
    echo "     --description=\"Firewall deny events\" \\"
    echo "     --filter=\"resource.type=gce_firewall_rule jsonPayload.disposition=DENIED\""
    echo
}
...
show_log_retention() {
    ...
    echo "     bigquery.googleapis.com/projects/PROJECT/datasets/DATASET \\"
    echo "     --log-filter=\"resource.type=cloud_run_job\""
    echo
}
...
echo
echo "🛠️ Enable logging where needed:"
